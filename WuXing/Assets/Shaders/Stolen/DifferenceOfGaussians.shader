Shader "Hidden/DifferenceOfGaussians" {
    Properties {
        ...
        _Sigma ("Sigma", Float) = 1.0          // Standard deviation for the first Gaussian
        _K ("K (Sigma Multiplier)", Float) = 1.6 // Scaling factor for the second Gaussian's sigma
        _GaussianKernelSize ("Gaussian Kernel Size", Int) = 3 // Size of the Gaussian kernel

        // Edge Detection Settings
        _Tau ("Tau (DoG Scale Factor)", Float) = 0.98 // Scaling factor for the second Gaussian in DoG calculation
        _Thresholding ("Enable Thresholding", Int) = 0 // 0 for false, 1 for true
        _Threshold ("Threshold", Float) = 0.1 // Threshold value for edge detection
        _Tanh ("Use Tanh Thresholding", Int) = 0 // 0 for binary, 1 for tanh
        _Phi ("Tanh Steepness", Float) = 1.0 // Steepness factor for tanh-based thresholding
        _Invert ("Invert Output", Int) = 0 // 0 for false, 1 for true
    }
